...
- Ao partir de uma matriz genérica de comprimentos `(Lx, Ly, Lz)` por elo, aplique `mask_excentricities_by_axis(axis_order)`
  para zerar automaticamente as componentes que não aparecem na matriz de excentricidades do MATLAB; apenas o eixo indicado
  em cada entrada de `axis_order` permanece diferente de zero.
